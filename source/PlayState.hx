...
class PlayState extends MusicBeatState
{
	...
	public var setIdle:String = 'idle';
	public var setdadIdle:String = 'idle';
	public var setgfIdle:String;
	...
	public static var whiteScanline:FlxSprite;
	public var camAnimeshoun : FlxSprite;
	public var staticAnimeshoun:FlxSprite;
	public var constantstaticAnimeshoun:FlxSprite;
	public static var invertWarn : FlxSprite;
	public var goodLuck:FlxSprite;
	public var nofriends:FlxSprite;
	public static var causeOfDeath:String = "unkown, please contact the developers";
	public static var lastWasA:String = 'UK';

	public var staticStage:FlxSprite;

	public var goodLuckdone:Bool = false;

	//static arrows
	public var babyArrow:FlxSprite;

	public var gfCanMove:Bool = true;
	public var bfCanMove:Bool = true;
	public var bfGotHurt:Bool = false;

	public var gfIsCheering:Bool = false;

	//checks if finale cutscenes are toggled
	public var cutFinale:Bool = false;

	//rubberbanding value
	public static var rubberValue:Float;
	public static var rubberRank:Float = 0;
	...
	override public function create()
	{
		...
		//uses the set arrow style
		//SONG.noteStyle = FlxG.save.data.arrowStyle;
		...
		if (PlayStateChangeables.botPlay)
			boyfriend = new Boyfriend(770, 450, 'stock');
		else
			boyfriend = new Boyfriend(770, 450, SONG.player1);

		// REPOSITIONING PER BF CHAR
		...
		osuOverlay = new FlxSprite().loadGraphic(Paths.image('osu/osu'));
		osuOverlay.scrollFactor.set();
		osuOverlay.antialiasing = true;
		osuOverlay.screenCenter();

		osuJudgment = new FlxSprite().loadGraphic(Paths.image('osu/osu_Judgment_Bar-' + FlxG.save.data.downscroll));
		osuJudgment.scrollFactor.set();
		osuJudgment.antialiasing = true;
		osuJudgment.screenCenter();
		...
		invertWarn = new FlxSprite().loadGraphic(Paths.image('overlays/invert/invert-' + Localizations.languageVar));
		invertWarn.scrollFactor.set();
		invertWarn.antialiasing = true;
		invertWarn.visible = false;
		invertWarn.screenCenter();
		...
		whiteScanline = new FlxSprite().loadGraphic(Paths.image('overlays/white-scanline'));
		whiteScanline.scrollFactor.set();
		whiteScanline.antialiasing = true;
		whiteScanline.screenCenter();
		whiteScanline.y = -600;
		...
		jsAnimeshoun.frames = Paths.getSparrowAtlas('overlays/jumpscare');
		...
		jsAnimeshoun.animation.play('idle');

		staticStage = new FlxSprite();
		staticStage.frames = Paths.getSparrowAtlas('static/static');
		staticStage.animation.addByPrefix('idle', 'idle', 24, true);
		staticStage.screenCenter();
		staticStage.scrollFactor.set();
		staticStage.animation.play('idle');
		staticStage.visible = false;
		...
		goodLuck = new FlxSprite().loadGraphic(Paths.image('overlays/good luck/good-luck-' + Localizations.languageVar));
		goodLuck.scrollFactor.set();
		goodLuck.antialiasing = true;
		goodLuck.screenCenter();
		goodLuck.visible = false;

		nofriends = new FlxSprite().loadGraphic(Paths.image('overlays/nofriends'));
		nofriends.scrollFactor.set();
		nofriends.antialiasing = true;
		...
		if (!PlayStateChangeables.Optimize)
		{
			...
			if (SONG.song == 'Thanatophobia' && isStoryMode)
				dad.visible = false;
			...
			if (curStage == 'woods' && FlxG.save.data.JumpScareSlender == true)
			{
				add(constantstaticAnimeshoun);
				add(staticAnimeshoun);
			}

			if (curStage == 'woods' && FlxG.save.data.UIcamSlender == true)
			{
				add(camAnimeshoun);
			}
			...
			if (curStage == 'woods' && FlxG.save.data.tvWhiteSlender == true)
			{
				add(whiteAnimeshoun);
				//add(whiteScanline);
			}

			if (curStage == 'woods' && FlxG.save.data.tvFilterSlender == true)
			{
				add(crtFilter);
			}

			if (SONG.noteStyle == 'osu-mania' && FlxG.save.data.osuOverlay == true)
			{
				add(osuOverlay);
			}

			if (SONG.noteStyle == 'osu-mania')
			{
				add(osuJudgment);
			}
		...
		/*if (SONG.song.toLowerCase() == 'thanatophobia' || isStoryMode)
			{
				persistentUpdate = false;
				persistentDraw = true;
				paused = true;
				DialogueSubState.song = 'thanatophobia';
		...
		if (PlayStateChangeables.botPlay)
			iconP1 = new HealthIcon('stock', true);
		else
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			switch (noteTypeCheck)
			{
				...
				case 'osu-mania':
					babyArrow.frames = Paths.getSparrowAtlas('NOTE_assets-osu');
					for (j in 0...4)
					{
				...
				case 'vanilla':
					babyArrow.frames = Paths.getSparrowAtlas('vanilla_NOTE_assets');
					for (j in 0...4)
					{
						babyArrow.animation.addByPrefix(dataColor[j], 'arrow' + dataSuffix[j]);	
					}
	...
	override public function update(elapsed:Float)
	{
		...
		if (FlxG.keys.justPressed.LEFT || FlxG.keys.justPressed.DOWN || FlxG.keys.justPressed.UP || FlxG.keys.justPressed.RIGHT)
		{
			if (FlxG.save.data.isSwitched)
			{
				causeOfDeath = Localizations.invKeyCheeseDeath;
				killBfRegardless();
			}
		}
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if (!daNote.mustPress && daNote.wasGoodHit)
				{
					...
					cpuStrums.forEach(function(spr:FlxSprite)
					{
						...
						{
							if (dad.curCharacter != 'pico-ememy')
							...
							if (health >= 0)
							{
								if (rubberValue <= 31.9)
									health -= 0.02;
									rubberRank = 2;
								if (rubberValue >= 32 && rubberValue <= 33.9)
									health -= 0.04;
									rubberRank = 3;
								if (rubberValue >= 34)
									health -= 0.05;
		...
		else
		{
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					//FlxG.save.data.completeWeekOneSlender = true;

					var overCover:FlxSprite = new FlxSprite(0, 0).makeGraphic(1280, 720, 0xFF000000);
					add(overCover);
					...
					FlxG.save.data.lastSong = SONG.song;
					...
					else
					{
						...
						FlxG.switchState(new GoodEndState());
					}
	...
	private function popUpScore(daNote:Note):Void
	{
		...
		switch (daRating)
		{
			case 'shit':
				...
				causeOfDeath = Localizations.shitDeath;
				killBf();
			case 'bad':
				lastWasA = 'BD';
				rubberValue -= 0.01;
				...
				causeOfDeath = Localizations.badDeath;

				if (storyDifficultyText.toLowerCase() == 'hard' || storyDifficultyText.toLowerCase() == 'normal')
					if (rubberValue >= 20)
						killBf();
			...
			case 'good':
				...
				causeOfDeath = Localizations.goodDeath;

				if (storyDifficultyText.toLowerCase() == 'hard')
					if (rubberValue >= 20)
						killBf();
			case 'sick':
				lastWasA = 'SK';
		...
		if (daRating != 'shit' || daRating != 'bad')
		{
			...
			{
				case 'shit':
					currentTimingShown.color = FlxColor.BROWN;
				case 'bad':
	...
	{
		causeOfDeath = Localizations.missDeath;
		lastWasA = 'MS';
		...
		if (!note.wasGoodHit)
		{
			...
			playerStrums.forEach(function(spr:FlxSprite)
			{
				if (Math.abs(note.noteData) == spr.ID)
				{
					...
					if (health < 2)
						health += (((0.04 / rubberValue) * 20) + 0.02);
				}
			});
	...
	{
		//trace ("new step");
		//trace ("rubber value: " + (rubberValue - 30));

		switch (SONG.song)
		{
			case 'Thanatophobia':
				causeOfDeath = Localizations.ukTxt + "1T" + ((health * 100) / 2) + lastWasA;
			case 'Static':
				causeOfDeath = Localizations.ukTxt + "1S" + ((health * 100) / 2) + lastWasA;
			case 'Haphephobia':
				causeOfDeath = Localizations.ukTxt + "1H" + ((health * 100) / 2) + lastWasA;
			case 'Engage':
				causeOfDeath = Localizations.ukTxt + "1E" + ((health * 100) / 2) + lastWasA;
		}
		...
		//JUMPSCARE EVENTS
		if (FlxG.save.data.JumpScareSlender == true)
		{
			switch (curSong)
			{
				case 'Thanatophobia':
				...
				case 'Engage':
				{
					switch (curStep)
					{
						...
						case 2560:
							...
							whiteAnimeshoun.visible = true;
							camAnimeshoun.visible = true;
							staTicEnd();					
					}
				}
	...
	override function beatHit()
	{
		...
		new FlxTimer().start(((Conductor.bpm / 60) / 10) * 2, function(tmr:FlxTimer)
			{
				FlxTween.tween(whiteScanline,{y: -600}, ((Conductor.bpm / 60) / 10) * 2, {ease: FlxEase.expoInOut});
			}, 0);
		trace (((Conductor.bpm / 60) / 10) * 2);
		...
		//EVENTS PER SONG, DIVIDED BY BEAT
		switch (SONG.song)
		{
			case 'Blammed remix':
				switch (curBeat)
		...
		if (!boyfriend.animation.curAnim.name.startsWith("sing") && (curBeat % idleBeat == 0/* || !idleToBeat*/))
		{
			...
			{
				if (bfGotHurt)
					boyfriend.playAnim('hurt loop', idleToBeat);
				else
				{
					boyfriend.playAnim(setIdle, idleToBeat);
				}
			}
		}
	...
	function killBfRegardless():Void
	{
		boyfriend.stunned = true;
	...
	function invertKeys():Void
	{
		invertWarn.visible = true;

		trace(FlxG.save.data.leftBind, FlxG.save.data.downBind, FlxG.save.data.upBind, FlxG.save.data.rightBind);

		FlxG.save.data.isSwitched = true;
		
		FlxG.save.data.upBind = FlxG.save.data.lastDown;
		FlxG.save.data.downBind = FlxG.save.data.lastUp;
		FlxG.save.data.leftBind = FlxG.save.data.lastRight;
		FlxG.save.data.rightBind = FlxG.save.data.lastLeft;
	...
	function constantStatic():Void
	{
		constantstaticAnimeshoun.visible = true;
		constantstaticAnimeshoun.alpha = 0.10;
		trace ("static");
	}

	function constantStaticEnd():Void
	{
		constantstaticAnimeshoun.visible = false;
		trace ("static end");
	}
	...
	function staTic():Void
	{
		...
		staticAnimeshoun.alpha = 0.20;
		trace ("static");
	}

	function staTicEnd():Void
	{
		staticAnimeshoun.visible = false;
		trace ("static end");
	}	

	function strongStat():Void
	{
		staticAnimeshoun.visible = true;
		staticAnimeshoun.alpha = 0.40;
		trace ("strong static");
	}

	function strongStatEnd():Void
	{
		staticAnimeshoun.visible = false;
		trace ("strong static end");
	}	

	function toggleCutFinale():Void
	{
		
		if (cutFinale == false)
			cutFinale = true;

		else if (cutFinale == true)
			cutFinale = false;
	}

	function osuManiaFinish():Void
	{
		osuOverlay.kill();
		osuJudgment.kill();
	}

	function coverPlayState():Void
	{
		var coverPS:FlxSprite = new FlxSprite(0, 0).makeGraphic(8000, 8000, 0xFF000000);
